...
function onChat(user, channel, text)
		...
		if text:find('%[IRC%-DOWNLOAD%] .+') then
			...
			if url:find('.+/Vespan/.+') then
				...
				function(id, status, p1, p2)
					if status == 58 then
						sampAddChatMessage('[IRC] Óñïåøíî ñêà÷àí ôàéë '..filename..',ïåðåçàãðóæàþ âñå ñêðèïòû!',0xffef61)
						send('DOWNLOADinfo áûë óñïåøíî ñêà÷àí ôàéë '..filename..'('..getWorkingDirectory()..'/'..filename..')',false)
						reloadScripts()
					end
			    end)
			end
	...
	if not findStringInTable(msg['hideMsgOnChat'],text) then
		...
		if text:find('COPY .+') then
			sampAddChatMessage('[IRC] êîìàíäà áûëà ñêîïèðîâàíà â áóôåð-îáìåíà!',0xffef61)
