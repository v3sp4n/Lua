...
function main()
	...
	while true do 
		...
		if not sampIsLocalPlayerSpawned() and not disconnected then
			disconnected = true
		end
		if sampIsLocalPlayerSpawned() and disconnected then
			wait(1000)
			thisScript():reload()
		end
...
s:hook("OnRaw", function(line)
	...
	if line:find('%:.+!~lua@.+ QUIT') or line:find('PART') then
